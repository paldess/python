osnov = {'Название': 0, 'Цена': 0, 'Количество': 0, 'Единица измерения': 0}  # если переставить данную строку чуть ниже
schet = 0
struktura = []
while True:
    act = input('Выберите действие: 1-добавить товар, 2-просмотреть товары, '
                '3-посмотреть количество товаров в базе, '
                '\n4-названия товаров, 5-общую стоимость, 6-общее количество, '
                '7-собрать аналитику, exit - для завершения сеанса: \n')
    if act == '1':
                                                                            # вот сюда, то программа работает корректно
        schet += 1
        tovar = [schet]
        osnov['Название'] = input('Введите название товара:  ')
        q = input('Введите цену товара:  ')
        while not q.replace('.', '').isdigit():
